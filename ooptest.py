...
class menu():
    def __init__(self):
        self.surface = pygame.Surface((sim.screen_width-sim.screen_height, sim.screen_height)) #sets up the surface for the menu to be on
        self.backgroundcolour = (10, 10, 10) #makes a variable to store the colour of the menu surface
...
class Simulation():
    ...
    def __init__(self):
        ...
        screen_info = pygame.display.Info() #gets the display information
        self.screen_width = screen_info.current_w #gets the width of the computer's monitor
        self.screen_height = screen_info.current_h #gets the height of the computer's monitor
        self.screen = pygame.display.set_mode((self.screen_width,self.screen_height)) #initiallizes the display for rendering
        self.box_dims = self.screen_height

        self.simBox = pygame.Surface((self.box_dims,self.box_dims))
        self.menu_Box = pygame.Surface((self.screen_width-self.screen_height, self.screen_height))
        self.menu_Box.fill((10, 10, 10))

        self.particle_num = 2000 #the number of particles which will spawn
        ...
        self.matrix = [[0,-1,0],
                       [-1,0,0],
                       [0.5,0.5,1]]#attraction matrix for how the colours interact

        self.max_radius = 0.15 #the distance at which particles can interact
        self.force_factor = 10 #increases the magnitude of reactions
        self.friction_factor = math.pow(0.5, 0.01 / 0.040) #friction
        self.edge_distance =20/self.box_dims #padding
        ...
        dll_path = os.path.join(os.path.dirname(__file__), "interaction.dll")
        self.interaction = ctypes.CDLL(dll_path)
        self.interaction.update_particles.argtypes = [
    ...
    def particleOnParticle(self):
        ...
        xpos = np.array([p.xpos for p in self.particles], dtype=np.float32)
        ypos = np.array([p.ypos for p in self.particles], dtype=np.float32)
        xvel = np.array([p.xvel for p in self.particles], dtype=np.float32)
        yvel = np.array([p.yvel for p in self.particles], dtype=np.float32)
        colours = np.array([p.colour for p in self.particles], dtype=np.int32)
        matrix = np.array(self.matrix, dtype=np.float32).flatten()

        self.interaction.update_particles(
            xpos.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
            ypos.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
            xvel.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
            yvel.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
            colours.ctypes.data_as(ctypes.POINTER(ctypes.c_int)),
            ctypes.c_int(self.particle_num),
            matrix.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
            ctypes.c_int(self.colour_num),
            ctypes.c_float(self.beta),
            ctypes.c_float(self.max_radius),
            ctypes.c_float(self.force_factor),
            ctypes.c_float(self.friction_factor)
        )
        ...
        for i, p in enumerate(self.particles):
            ...
            p.xvel = xvel[i]
            p.yvel = yvel[i]
            # ...existing code for drawing and wrapping...

            # scales the position of the particle to the screen dimensions
            self.particles[i].xpos = (self.box_dims * self.particles[i].xpos)
            self.particles[i].ypos *= self.box_dims

            # screen wrapping
            if self.particles[i].xpos > self.box_dims:  # checks if the particle is off the right side of the simulation area
                self.particles[i].xpos = (self.particles[i].xpos % self.box_dims)

            if self.particles[i].xpos < self.box_dims:
                self.particles[i].xpos = (self.particles[i].xpos % self.box_dims)

            if self.particles[i].ypos < self.box_dims:
                self.particles[i].ypos = (self.particles[i].ypos % self.box_dims)
            ...
            self.particles[i].xpos /= self.box_dims
            self.particles[i].ypos /= self.box_dims
